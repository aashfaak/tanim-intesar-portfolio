export const site = {
  name: "TANIM Intesar",
  shortName: "INTESAR",
  tagline: "Student. Observer. Learner.",
  role: "IBA Student",
  url: "https://tanimintesar.example.com",
  description:
    "A running record of university life, ideas, travel, and the things worth thinking about — written by an IBA student.",
  email: "hello@ashfakrahman.example.com",
  nav: [
    { label: "About", href: "/about" },
    { label: "Journal", href: "/journal" },
    { label: "Travel", href: "/travel" },
    { label: "Blog", href: "/blog" },
    { label: "Contact", href: "/contact" },
  ],
  social: [
    { label: "Instagram", href: "https://www.instagram.com/tanim.015?stkn=MXF2a2ZmOTdxYjJqZQ==" },
    { label: "LinkedIn", href: "https://linkedin.com/" },
    { label: "Facebook", href: "https://www.facebook.com/profile.php?id=61578486527634" },
  ],
  currently: [
    { label: "Studying", value: "Business & Management, IBA" },
    { label: "Learning", value: "Digital Strategy" },
    { label: "Reading", value: "The Almanack of Naval Ravikant" },
    { label: "Exploring", value: "New places and ideas" },
    { label: "Building", value: "Personal projects" },
  ],
};
